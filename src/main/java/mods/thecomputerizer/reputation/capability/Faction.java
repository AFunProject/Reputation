...
public class Faction {
	...
	public Faction(ResourceLocation id, String name, int defaultRep, int lowerRep, int higherRep,
		...
		this.currencyItem = Objects.nonNull(currencyItem) ? currencyItem : DIAMOND;
	...
	public ResourceLocation getID() {
		return Objects.nonNull(this.id) ? this.id : ReputationRef.res("");
	}
	...
	public static Faction fromJsonAsString(String identifier, String jsonString) {
		try {
			...
			ResourceLocation id;
			if(!name.contains(":")) id = ReputationRef.res(json.get("name").getAsString());
	...
	public static Faction fromJson(ResourceLocation id, JsonElement data) {
		try {
			...
			if(!name.contains(":")) faction_id = ReputationRef.res(json.get("name").getAsString());
			else {
			...
			int playerJoin = json.has("player_join_threshold") ?
					json.get("player_join_threshold").getAsInt() : MIN_VALUE;
			Item currency = ITEMS.getValue(new ResourceLocation(json.get("currency").getAsString()));
			Map<String, Integer> weighting = new HashMap<>();
			...
			return new Faction(faction_id,name,defaultRep,lowerRep,higherRep,playerJoin,
							   currency,weighting,enemies,members);
		} catch(Exception ex) {
			throw new RuntimeException("Failed to parse faction with id "+id+"!",ex);
		}
	}
	...
	private static List<EntityType<?>> parseMembers(JsonObject json) {
		...
		if(json.has("members")) {
			for(ResourceLocation loc : parseResourceArray("members",json)) {
				EntityType<?> entity = ENTITIES.getValue(loc);
				members.add(entity);
			}
		}
		return members;
	...
	public String toJsonString() {
		...
		builder.append("\"currency\": \"").append(Objects.requireNonNull(this.currencyItem.getRegistryName()))
				.append("\", ").append("\n");
