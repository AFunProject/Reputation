...
public class ReputationCapability implements IReputation {
	...
	@Override public Map<Faction,Integer> allReputations() {
		return FACTIONS;
	}

	@Override public int getReputation(Faction faction) {
		if(Objects.isNull(faction)) return 0;
		if(!FACTION_IDS.containsKey(faction.getID().toString()))
			FACTION_IDS.put(faction.getID().toString(),faction.getDefaultRep());
		return FACTION_IDS.get(faction.getID().toString());
	}

	@Override public void setReputation(Player p, Faction faction, int reputation) {
		...
		if(p instanceof ServerPlayer player)
			ReputationNetwork.sendToClient(new PacketSyncReputation(faction.getID(),reputation),player);
	}

	@Override public void changeReputation(Player p, Faction faction, int reputation) {
		if(Objects.isNull(p) || Objects.isNull(faction)) return;
		int base = FACTION_IDS.containsKey(faction.getID().toString()) ?
				FACTION_IDS.get(faction.getID().toString()) : faction.getDefaultRep();
		FACTION_IDS.put(faction.getID().toString(),base+reputation);
		FACTIONS.put(faction,base+reputation);
		if(p instanceof ServerPlayer player)
			ReputationNetwork.sendToClient(new PacketSyncReputation(faction.getID(),base+reputation),player);
	}

	@Override public CompoundTag writeTag(CompoundTag tag) {
	...
	@Override public void readTag(CompoundTag tag) {
