...
import static mods.thecomputerizer.reputation.registry.SoundRegistry.LEDGER_PLACE;
import static net.minecraft.sounds.SoundSource.BLOCKS;
import static net.minecraft.world.inventory.MenuType.GENERIC_9x3;
...
public class LedgerEntity extends BaseContainerBlockEntity implements LidBlockEntity {
    ...
    public static <T> void tick(Level level, BlockPos pos, BlockState ignored, T ledger) {
        ((LedgerEntity)ledger).tick(level,pos);
    }

    private int cooldownTimer = 0;
    protected NonNullList<ItemStack> items = NonNullList.withSize(27,EMPTY);
    
    public LedgerEntity(BlockPos pos, BlockState state) {
        this(LEDGER_ENTITY.get(),pos,state);
    }
    ...
    @Override public void load(@NotNull CompoundTag tag) {
        ...
        if(tag.contains("ledger_cooldown")) this.cooldownTimer = tag.getInt("ledger_cooldown");
    }
    ...
    protected void runCollection(Level level, BlockPos pos) {
        for(ItemStack stack : this.items) {
            if(!stack.isEmpty() && stack.getItem()==FACTION_BAG.get()) {
        ...
        level.playLocalSound(pos.getX(),pos.getY(),pos.getZ(),LEDGER_PLACE.get(),BLOCKS,1f,
    ...
    @Override protected @NotNull AbstractContainerMenu createMenu(int containerId, @NotNull Inventory inventory) {
        return new Menu(GENERIC_9x3,containerId,this,inventory);
    }
    ...
    @Override public @NotNull ItemStack getItem(int index) {
        return this.items.get(index);
    }

    @Override public @NotNull ItemStack removeItem(int index, int count) {
        return getItem(index);
    }

    @Override public @NotNull ItemStack removeItemNoUpdate(int index) {
        return getItem(index);
    }
    ...
    @Override public float getOpenNess(float pPartialTicks) {
        return 0;
    }
    ...
    private static class Menu extends AbstractContainerMenu {
        ...
        protected Menu(@Nullable MenuType<?> menuType, int containerId, LedgerEntity container, Inventory inventory) {
            ...
            for(int i=0;i<3;i++)
                for(int j=0;j<9;j++) this.addSlot(new LedgerSlot(container,j+i*9,8+j*18,18+i*18));
            for(int i=0;i<3;i++)
                for(int j=0;j<9;j++) this.addSlot(new Slot(inventory,j+i*9+9,8+j*18,85+i*18));
            for(int i=0;i<9;i++) this.addSlot(new Slot(inventory,i,8+i*18,143));
        }
