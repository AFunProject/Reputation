...
public class FleeBattleGoal extends Goal {
    ...
    private boolean ensureCorrectConditions(Player player) {
        return (this.mob.getHealth()/this.mob.getMaxHealth()) <= 0.5f
                ...
                && HelperMethods.isPlayerInCustomStanding(this.mob,player,STANDINGS.getInjured(this.mob.getType()));
    }
    ...
    @Override public boolean canUse() {
        if(this.mob.getLastHurtByMob() instanceof Player player) {
            if(ensureCorrectConditions(player)) {
                ...
                ReputationNetwork.sendToClient(new PacketFleeIcon(mob.getUUID(),true),player);
    ...
    @Override public void stop() {
        if(!this.mob.isRemoved() && !ensureCorrectConditions(this.fleeFrom))
            ReputationNetwork.sendToClient(new PacketFleeIcon(mob.getUUID(),false),this.fleeFrom);
        this.fleeFrom = null;
    }
    ...
    @Override public void tick() {
        if(!escaped()) {
            ...
            synchronized(TRACKER_MAP) {
                if(TRACKER_MAP.containsKey(mob)) {
