...
public class LedgerEntity extends BaseContainerBlockEntity implements LidBlockEntity {
    ...
    protected void runCollection(Level level, BlockPos pos) {
        for (ItemStack stack : this.items) {
            if (stack != ItemStack.EMPTY && stack.getItem() == Items.FACTION_BAG.get()) {
        ...
        level.playLocalSound(pos.getX(),pos.getY(),pos.getZ(), Sounds.LEDGER_PLACE.get(), SoundSource.BLOCKS,
                1f, Mth.randomBetween(ReputationHandler.RANDOM,0.88f,1.12f),
                false);
        this.clearContent();
        this.setCustomName(new TranslatableComponent("ledger.reputation.checked"));
    ...
    public boolean stillValid(@NotNull Player player) {
        if (Objects.nonNull(this.level) && this.level.getBlockEntity(this.worldPosition) != this) {
            return false;
        } else {
    ...
    private static final class LedgerSlot extends Slot {
        ...
        public boolean mayPlace(@NotNull ItemStack stack) {
            return this.getItem()==ItemStack.EMPTY;
        }
